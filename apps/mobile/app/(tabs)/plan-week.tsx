...
function buildWeekDays(
  ...
  for (let i = 0; i < 7; i++) {
    ...
    const dayOutfit =
      scheduledOutfits.find(
        o =>
          o.scheduled_date === dateStr || (o.event_id && dayEvents.some(e => e.id === o.event_id))
      ) || null;
...
export default function PlanWeekScreen() {
  ...
  const areSuggestionsEquivalent = useCallback(
    (first: OutfitSuggestion | null, second: OutfitSuggestion | null) => {
      if (!first || !second) return false;

      const firstItems = [...first.items].sort();
      const secondItems = [...second.items].sort();

      return (
        first.name === second.name &&
        first.rationale === second.rationale &&
        firstItems.length === secondItems.length &&
        firstItems.every((itemId, index) => itemId === secondItems[index])
      );
    },
    []
  );
  ...
  const loadData = useCallback(async () => {
    ...
    const weekDays = buildWeekDays(eventsResult.events, scheduledResult.outfits, forecast);
    setDays(weekDays);
    setSelectedDay(current => current ?? weekDays[0]);
    setIsLoading(false);
  }, [forecast]);
  ...
  const handleGenerateSuggestion = async (forceFresh: boolean = false) => {
    if (!selectedDay) return;
    setIsSuggesting(true);

    // Use highest-formality event if available
    const topEvent = [...selectedDay.events].sort(
      (a, b) => (b.formality_score || 0) - (a.formality_score || 0)
    )[0];

    if (topEvent) {
      if (forceFresh) {
        await clearEventOutfitCache(topEvent.id);
      }
      const result = await generateEventOutfit(topEvent, wardrobeItems, forceFresh);
      setSuggestion(result.suggestion);
    } else {
      const currentSuggestion = suggestion;
      const { suggestions } = await generateOutfitSuggestions(wardrobeItems);
      const nextSuggestion =
        suggestions?.find(candidate => !areSuggestionsEquivalent(candidate, currentSuggestion)) ||
        suggestions?.[0] ||
        generateFallbackOutfit(wardrobeItems);
  ...
  const handlePickOutfit = async (outfit: Outfit) => {
    ...
    const itemIds = outfit.items?.map(item => item.item_id) || [];
              ...
              >
                ...
                {day.forecast ? (
                  <View style={styles.weatherChip}>
                    <Ionicons
                      name={getWeatherIcon(day.forecast.condition)}
                      size={14}
                      color="#6b7280"
                    />
                ...
                <View style={styles.eventsSection}>
                  ...
                  {selectedDay.events.map(event => (
                    <View key={event.id} style={styles.miniEvent}>
                      <Ionicons name="calendar-outline" size={16} color="#5D4E37" />
                ...
                <View style={styles.scheduledSection}>
                  ...
                  <View style={styles.outfitCard}>
                    <View style={styles.outfitItemGrid}>
                      {getItemsForOutfit(selectedDay.scheduledOutfit)
                        .slice(0, 4)
                        .map(item => (
                    ...
                    <View style={styles.suggestionCard}>
                      ...
                      <View style={styles.suggestionActions}>
                        <TouchableOpacity
                          ...
                          onPress={() => handleGenerateSuggestion(true)}
                        ...
                        <TouchableOpacity
                          ...
                          disabled={isScheduling || isSuggesting}
                        >
                          {isScheduling ? (
        ...
        <View style={styles.pickerContainer}>
          ...
          {savedOutfits.length === 0 ? (
            <View style={styles.emptyPicker}>
              ...
              <Text style={styles.emptyPickerText}>Create outfits in the Outfits tab first.</Text>
            </View>
          ) : (
            <FlatList
              data={savedOutfits}
              ...
              renderItem={({ item: outfit }) => (
                ...
                >
                  <View style={styles.pickerOutfitInfo}>
                    <Text style={styles.pickerOutfitName}>{outfit.name || 'Untitled Outfit'}</Text>
                    {outfit.occasion && (
                      <Text style={styles.pickerOutfitOccasion}>{outfit.occasion}</Text>
                    )}
                  </View>
