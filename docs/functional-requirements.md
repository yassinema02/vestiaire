...
| FR-AUTH-05 | The system shall persist authenticated sessions securely using device keychain (expo-secure-store) | P0 |
...
| FR-AUTH-10 | The system shall create a `profiles` record automatically upon user registration via database trigger | P0 |
...
| FR-WRD-04 | The system shall automatically remove image backgrounds via the remove.bg API | P0 |
...
| FR-CTX-04 | Weather data shall be cached for 30 minutes with AsyncStorage persistence | P1 |
...
| FR-CTX-07 | Users shall connect their Apple Calendar (EventKit) to the app with permission explanation | P1 |
...
| FR-GAP-06 | Gap results shall be cached in AsyncStorage and refresh when wardrobe changes | P2 |
...
| ID | Requirement | Priority |
|----|-------------|----------|
| FR-PSH-01 | The system shall request push notification permission on iOS via expo-notifications | P0 |
...
| ID | Requirement |
|----|-------------|
| NFR-SEC-01 | All API keys (Gemini, OpenAI, remove.bg) shall be stored server-side only, never exposed to the client |
| NFR-SEC-02 | All database tables shall enforce Row-Level Security (RLS) scoped to `auth.uid()` |
| NFR-SEC-03 | Session tokens shall be stored in iOS Keychain via expo-secure-store, not AsyncStorage |
...
| NFR-ACC-03 | Secondary platform: Android (via React Native) |
...
Photo Capture → Image Compression (on-device, 512px, 85% JPEG)
    → Upload to Storage (wardrobe-images/{user_id}/)
    → Background Removal (server-side via remove.bg API)
...
|-------------|---------|--------|-----------------|
| **Supabase Auth** | Authentication (email, Apple, Google) | SDK | N/A (managed) |
| **Supabase PostgreSQL** | Primary database | SDK (PostgREST) | Anon key (public, RLS-protected) |
| **Supabase Storage** | Image storage (wardrobe photos) | SDK | Anon key |
| **Supabase Edge Functions** | Server-side AI proxy, background removal | SDK `functions.invoke()` | Server-side secrets |
| **Google Gemini 2.0 Flash** | Clothing analysis, outfit generation, event classification, gap analysis, product extraction | Via Edge Function | Server-side secret |
| **remove.bg API** | Background removal from clothing photos | Via Edge Function | Server-side secret |
| **Open-Meteo API** | Weather data (current + forecast) | Direct HTTP (free, no key) | N/A |
| **Apple EventKit** | iOS calendar event sync | Expo Calendar module | N/A (on-device) |
| **Expo Push Notifications** | Push notification delivery (APNs/FCM) | Expo Push API | Expo project config |
| **Apple In-App Purchase** | Premium subscription billing | RevenueCat SDK (planned) | App Store Connect |
...
- Trial grant is atomic via Supabase RPC

### 9.4 Billing Implementation Status

- **Current:** Simulated (sets `premium_until` to +30 days)
- **Planned:** RevenueCat SDK integration for App Store subscription management
...
| **Data residency** | All data stored in EU region (Supabase eu-west-2 or eu-central-1) |
| **Right to access (DSAR)** | Export all user data as JSON via Edge Function |
...
|-----------|---------------|---------|-----------|
| Email, name | PII | Supabase Auth + profiles | Until account deletion |
| Wardrobe photos | Personal data | Supabase Storage (private bucket) | Until account deletion |
| Wear logs | Usage data | PostgreSQL | Until account deletion |
| AI analysis results | Derived data | PostgreSQL | Until item deletion |
| Push tokens | Device identifier | profiles table | Until logout/deletion |
| Calendar events | Synced data | PostgreSQL | 30-day rolling window |
